                ...
                for (const printifyOrder of printifyOrdersPage.data) {
                    ...
                    if (order) {
                        if (order['status'] !== printifyOrder.status) {
                            await ordersCollection.updateOne({ orderID: printifyOrder.id }, { $set: { status: printifyOrder.status } });
                        }
                        if (printifyOrder.status === "in-production" && !order['emailOrderCreated']) {
                            await sendEmail(printifyOrder);
                            await ordersCollection.updateOne({ orderID: printifyOrder.id }, { $set: { emailOrderCreated: true } });
                        }
                        if (printifyOrder.status === "fulfilled" && !order['emailOrderShipped']) {
                            await sendEmail(printifyOrder);
                            await ordersCollection.updateOne({ orderID: printifyOrder.id }, { $set: { emailOrderShipped: true } });
                        }
                        if (printifyOrder.status === "canceled" && !order['emailOrderCanceled']) {
                            await sendEmail(printifyOrder);
                            await ordersCollection.updateOne({ orderID: printifyOrder.id }, { $set: { emailOrderCanceled: true } });
                        }
                    }
                }
            }
