...
export const fetchAdminProducts = functions.https.onRequest(
  { secrets: ["PRINTIFY_STORE_ID", "PRINTIFY_URL", "PRINTIFY_API_KEY"] },
  async (req, res) => {
    corsHandler(req, res, async () => {
      try {
        ...
        const sd_sticker_url = `${PRINTIFY_SHOP_URL}/shops/${shop_id}/products/699c6d2bd5a7dc05d50c4305.json`;
        const all_product_urls = [rmd_sticker_url, rmc_sticker_url, rmd_magnet_url, rmc_magnet_url, sd_sticker_url];
