...
export const getTrucksByBusiness = functions.https.onRequest({ secrets: ["MONGO_URI"] }, async (req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      await client.close();
      res.status(200).json({ message: "Trucks retrieved successfully!", trucks });
    } catch (error: any) {
      console.error("Error getting trucks:", error);
      res.status(500).json({ message: "Error getting trucks", error: error.message });
    }
  });
});
