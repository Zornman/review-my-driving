    ...
    initializeSubmissions() {
        ...
        this.dbService.getUserSubmissions(this.user?.uid).subscribe({
            next: (response) => {
                ...
                this.todaysSubmissions = response.submissions.filter((x) => {
                ...
                this.lastMonthSubmissions = response.submissions.filter((x) => {
                    const date = this.parseDate(x.dateSubmitted);
                    date.setHours(0, 0, 0, 0);
...
], AccountOverviewComponent.prototype, "paginator", void 0);
